...
SETTINGS_FILE = "../setting.txt" # Assuming the script is in a subdirectory and setting.txt is in the parent.
...
# --- GraphQL Query (for fetching PR commit list) ---
PR_COMMITS_QUERY = """
...
def run_graphql_query(query, variables):
    """Executes a GraphQL query and returns the resulting JSON."""
        ...
        try:
            ...
            if 'errors' in result and result['errors']:
                if any("RATE_LIMITED" in err.get("type", "") for err in result["errors"]):
                    ...
                    print(f"Warning: GraphQL rate limit hit. Waiting for {wait_time} seconds...")
                    time.sleep(wait_time)
                    if attempt < 2:
                        continue
                    else:
                        raise Exception(f"GraphQL RATE_LIMITED after failed retries: {result['errors']}")

                if any("NOT_FOUND" in err.get("type", "") for err in result["errors"]):
                    print(f"Warning: GraphQL entity not found. Error: {result['errors']}")
    ...
    raise Exception("Failed to execute GraphQL query (retry limit exceeded).")


def get_files_changed_in_single_commit_rest(owner, repo_name, commit_sha):
    """Fetches the list of files changed in a single specified commit using the REST API."""
    ...
    for attempt in range(3):
        try:
            ...
            if response.status_code == 403 and 'X-RateLimit-Remaining' in response.headers and int(
                ...
                print(f"Warning: REST API rate limit hit. Waiting for {wait_duration} seconds...")
                time.sleep(wait_duration)
                if attempt < 2:
                    continue
                else:
                    print("REST API rate limit: Retry limit exceeded.")
                    return []
            ...
            if e.response.status_code == 404:
                ...
                print(f"Warning: Unprocessable entity for commit {commit_sha} (REST) (422 Error). May be too large. URL: {url}")
                return []
            else:
                print(
                    f"REST API HTTP Error (commit {commit_sha}, attempt {attempt + 1}): {e.response.status_code} {e.response.reason}")
                if attempt == 2:
                    print(f"  HTTP Error: Retry limit exceeded. Response: {e.response.text if e.response else 'N/A'}")
                    return []
        except requests.exceptions.RequestException as e_req:
            print(f"REST API request error (commit {commit_sha}, attempt {attempt + 1}): {e_req}")
            if attempt == 2:
                print(f"  Request Error: Retry limit exceeded.")
                return []
...
def get_pr_commit_shas_sorted(owner, repo_name, pr_number):
    ...
    while True:
        ...
        if result.get("data") is None and result.get("errors"):
            print(
                f"Warning: Error fetching GraphQL commit info for PR #{pr_number} ({owner}/{repo_name}). Error: {result.get('errors')}")
            return []
        ...
        if not pr_data or 'commits' not in pr_data:
            if pr_data is None and result.get("data", {}).get("repository") is not None:
                ...
                print(f"Warning: Could not fetch GraphQL commit info for PR #{pr_number} ({owner}/{repo_name}).")
            break
        ...
        for node in commits_data.get('nodes', []):
            ...
            if isinstance(commit_info, dict) and 'oid' in commit_info and 'authoredDate' in commit_info:
                ...
                print(f"Warning: Malformed GraphQL commit node for PR #{pr_number}: {node}")
...
def analyze_pr_from_csv_row(row_data, csv_writer):
    ...
    if not all([owner, repo_name, pull_number_str]):
        print(f"Error: Missing PR identification info (owner, repo_name, pull_number). Skipping: {row_data}")
        return
    ...
    # --- Get total_changefile ---
    # total_changefile will be the unique changed files across the entire PR
    ...
    if len(pr_all_commit_shas) > 1:
        ...
        print(f"  Fetching changed files for subsequent commits ({len(commits_for_subsequent_analysis)} commits)...")
    ...
    print(f"  Analysis complete and written to CSV: {owner}/{repo_name}#{pr_number}")


def main():
    input_csv_file = '../Data/RQ1-HPR.csv'
    # Changed output filename back to the user-specified CSV name
    output_csv_file = '../Data/RQ2-HPR-changefile-Name.csv'

    if not GITHUB_TOKEN:
        print("Error: GITHUB_TOKEN environment variable is not set. Exiting script.")
        return
    ...
    try:
        with open(input_csv_file, mode='r', encoding='utf-8-sig', newline='') as infile, \
            ...
            if not reader.fieldnames:
                print(f"Error: CSV file '{input_csv_file}' is empty or has no header.")
                return

            # Check if required columns are in the input CSV (including 'url')
        ...
        print(f"\nAll processing is complete. Results have been output to '{output_csv_file}'.")

    except FileNotFoundError:
        print(f"Error: Input CSV file '{input_csv_file}' not found.")
        print(f"Current working directory: {os.getcwd()}")
        print("Please place the CSV file in the correct directory or update the path in the script.")
